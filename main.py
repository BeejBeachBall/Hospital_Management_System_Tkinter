...
class Hospital:
    def __init__(self,root):
        ...
        def iPrescription():
            ...
            self.txtPrescription.insert(END,'Reference Number: \t\t\t\t'+ Ref.get() + "\n")
            self.txtPrescription.insert(END,'Dose: \t\t\t\t'+ Dose.get() + "\n")
            self.txtPrescription.insert(END,'Number of Tab: \t\t\t\t'+ NumberTablets.get() + "\n")
        ...
        def iPrescriptionData():
            self.txtFrameDetail.insert(END, cmbNameTablets.get()+"\t"+ Ref.get()+ "\t"+Dose.get()+ "\t" +NumberTablets.get()+"\t"+Lot.get()+"\t"+IssuedDate.get()+"\t"+ExpDate.get()+"\t"+StorageAdvice.get()+"\t"+PatientNHSNo.get()+"\t"+PatientID.get()+"\t"+DateofBirth.get()+"\t"+PatientAddress.get()+"\n")
        ...
        TitleFrame = Frame(MainFrame, bd=20, width=2500, padx=20, relief=GROOVE)
        TitleFrame.pack(side=TOP)

        self.lblTitle = Label (TitleFrame, font=('arial', 40, 'bold'), text="KCDC Management System", padx=2)
        self.lblTitle.grid()

        FrameDetail = Frame(MainFrame, bd=20, width=1350, height=100, padx=20, relief=GROOVE)
        FrameDetail.pack(side=BOTTOM)

        ButtonFrame = Frame(MainFrame, bd=20, width=2500, height=50, padx=20, relief=GROOVE)
        ButtonFrame.pack(side=BOTTOM)

        DataFrame = Frame(MainFrame, bd=20, width=2500, height=400, padx=20, relief=GROOVE)
        DataFrame.pack(side=BOTTOM)

        DataFrameLEFT = LabelFrame(DataFrame, bd=10, width=800, height=300, padx=20, relief=GROOVE, font=('arial', 12, 'bold'), text ="Patient Informatioon")
        DataFrameLEFT.pack(side=LEFT)

        DataFrameRIGHT = LabelFrame(DataFrame, bd=10, width=450, height=300, padx=20, relief=GROOVE, font=('arial', 12, 'bold'), text="Prescription")
        ...
        self.lblDose = Label (DataFrameLEFT, font=('arial', 12, 'bold'), text="Dosage: ", padx=2, pady=2)
        ...
        self.lblUseMedication = Label (DataFrameLEFT, font=('arial', 12, 'bold'), text="Method To Take", padx=2, pady=2)
        ...
        self.lblPatientID = Label (DataFrameLEFT, font=('arial', 12, 'bold'), text="Lot: ", padx=2, pady=2)
        ...
        self.lblDaileyDose = Label (DataFrameLEFT, font=('arial', 12, 'bold'), text="Daily Dose: ", padx=2, pady=2)
        ...
        self.lblExpDate = Label (DataFrameLEFT, font=('arial', 12, 'bold'), text="Expire Date", padx=2, pady=2)
        ...
        self.btnPrescriptionData=Button(ButtonFrame, text='Prescription Data', font=('arial', 12, 'bold'), width=15, bd=4, command=iPrescriptionData)
        ...
        #====================Frame Detail=====================================================
        self.lblLabel = Label(FrameDetail, font=('arial', 10, 'bold'), pady=8,text="Tablet Name\tReference No.\t Dosage\t No. of Tablets\t Lot\t Issued Date\t Exp. Date\t Daily Dose\t Storage Advice\t NHS Number\t Patient Name\t DOB\t Address")
        self.lblLabel.grid(row=0, column=0)

        self.txtFrameDetail =Text(FrameDetail, font=('arial', 12, 'bold'), width=141, height=8, padx=2)
